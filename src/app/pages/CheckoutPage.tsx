...
import { ArrowLeft, Printer, Check, Banknote, Smartphone, MessageCircle } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'cash' | 'upi' | ''>(order?.paymentMethod || '');
  ...
  // Build a live order object with current payment state for WhatsApp
    ...
    const html = `<!DOCTYPE html><html><head><title>Receipt #${order.id}</title>
      ...
      <div class="receipt">
        ...
        <div style="margin-top:12px;text-align:center;padding:8px;border-radius:6px;font-size:14px;font-weight:600;${paid ? 'background:#DCFCE7;color:#166534' : 'background:#FEF2F2;color:#991B1B'}">
          ${paid ? `✅ PAID via ${paymentMethod === 'upi' ? 'UPI' : 'Cash'}` : '⏳ UNPAID'}
        </div>
      ...
      <div className="bg-white rounded-[16px] shadow-[0_1px_3px_rgba(0,0,0,0.08),0_4px_16px_rgba(0,0,0,0.04)] p-6 lg:p-8">
        ...
        <div className="space-y-3">
          {!completed ? (
            ...
            >
              <Check size={20} /> Complete Order{paid ? ` — ₹${order.total} ${paymentMethod === 'upi' ? 'via UPI' : paymentMethod === 'cash' ? 'via Cash' : ''}` : ' (Unpaid)'}
            </Button>
          ) : (
            <>
              <div className="text-center p-5 bg-[#DCFCE7] rounded-[10px] font-['DM_Sans'] text-[#166534]" style={{ fontWeight: 600 }}>
                ...
                {paid && <div className="text-[14px] mt-1 opacity-80">Paid ₹{order.total} via {paymentMethod === 'upi' ? 'UPI' : 'Cash'}</div>}
              </div>
              ...
              <a
                ...
                style={{ fontWeight: 600 }}
              >
                <MessageCircle size={20} /> Send Bill on WhatsApp
              </a>
